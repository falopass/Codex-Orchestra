...
import { readUserProviders } from "./user-providers.mjs";
...
// Overlay entries reuse the same rules as config/ providers, but a broken
// user-providers.json row must never take the Router down.
function userProviderProblem(provider) {
  if (!provider || typeof provider !== "object" || Array.isArray(provider)) {
    return "every provider must be an object";
  }
  if (!/^[a-z0-9][a-z0-9-]*$/.test(provider.id || "")) {
    return `invalid provider id ${JSON.stringify(provider.id)}`;
  }
  if (provider.kind !== "openai-compatible") {
    return `unsupported provider kind ${provider.kind} for ${provider.id}`;
  }
  for (const field of ["displayName", "ownedBy"]) {
    if (typeof provider[field] !== "string" || !provider[field]) {
      return `provider ${provider.id} requires ${field}`;
    }
  }
  if (provider.authProfile !== undefined) {
    return `provider ${provider.id} has an unsupported auth profile`;
  }
  if (provider.keyless !== undefined && typeof provider.keyless !== "boolean") {
    return `provider ${provider.id} has an invalid keyless flag`;
  }
  if (provider.keyless) {
    if (provider.credential !== undefined) {
      return `keyless provider ${provider.id} must not declare a credential`;
    }
    if (!/^https?:\/\/(127\.0\.0\.1|\[::1\]|localhost)([:/]|$)/.test(provider.baseUrl || "")) {
      return `keyless provider ${provider.id} must use a loopback baseUrl`;
    }
  } else if (!/^https:\/\//.test(provider.baseUrl || "")) {
    return `provider ${provider.id} requires an HTTPS baseUrl`;
  }
  if (
    !provider.keyless &&
    (!provider.credential?.file || !Array.isArray(provider.credential.environment))
  ) {
    return `provider ${provider.id} requires credential metadata`;
  }
  if (provider.credential !== undefined) {
    if (!singlePathSegment(provider.credential.file)) {
      return `provider ${provider.id} credential.file must be a single path segment`;
    }
    if (
      !Array.isArray(provider.credential.environment) ||
      provider.credential.environment.length === 0 ||
      provider.credential.environment.some(
        (name) => typeof name !== "string" || !/^[A-Z][A-Z0-9_]*$/.test(name),
      )
    ) {
      return `provider ${provider.id} credential.environment must be env names`;
    }
    if (
      provider.credential.label !== undefined &&
      (typeof provider.credential.label !== "string" || !provider.credential.label.trim())
    ) {
      return `provider ${provider.id} has an invalid credential label`;
    }
    if (
      Array.isArray(provider.credential.legacyFiles) &&
      provider.credential.legacyFiles.some((name) => !singlePathSegment(name))
    ) {
      return `provider ${provider.id} has an invalid credential.legacyFiles entry`;
    }
  }
  ...
  if (
    ...
    return `provider ${provider.id} has an invalid planNote`;
  }
  if (
    provider.protocol !== undefined &&
    !["openai", "anthropic", "openai-responses"].includes(provider.protocol)
  ) {
    return `provider ${provider.id} has an unsupported API protocol`;
  }
  if (provider.requestProfile !== undefined && typeof provider.requestProfile !== "string") {
    return `provider ${provider.id} has an invalid requestProfile`;
  }
  return undefined;
}

function mergeUserProviders(firstPartyProviders) {
  const warnings = [];
  const firstPartyIds = new Set(
    firstPartyProviders
      .map((provider) => (provider && typeof provider === "object" ? provider.id : undefined))
      .filter(Boolean),
  );
  const accepted = [];
  const acceptedIds = new Set();
  for (const provider of readUserProviders()) {
    if (!provider || typeof provider !== "object" || Array.isArray(provider)) {
      warnings.push("Skipped user provider: every provider must be an object");
      continue;
    }
    if (firstPartyIds.has(provider.id)) {
      warnings.push(`Skipped user provider: ${provider.id} is a first-party registry id`);
      continue;
    }
    if (acceptedIds.has(provider.id)) {
      warnings.push(`Skipped user provider: duplicate provider id ${provider.id}`);
      continue;
    }
    const problem = userProviderProblem(provider);
    if (problem) {
      warnings.push(`Skipped user provider: ${problem}`);
      continue;
    }
    accepted.push(provider);
    acceptedIds.add(provider.id);
  }
  return { providers: accepted, warnings };
}
...
function loadRegistry() {
  ...
  const userProviders = mergeUserProviders(parsed.providers);
  parsed.providers = [...parsed.providers, ...userProviders.providers];
  ...
  return {
    ...
    userProviderWarnings: Object.freeze(userProviders.warnings),
  };
}
...
export const USER_PROVIDER_WARNINGS = registry.userProviderWarnings;
